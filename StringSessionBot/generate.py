...
ERROR_MESSAGE = "Oops! An exception occurred! \n\n**Error** : {} " \
            "\n\nPlease Report to @teamsupportgroup if there is an error " \
...
async def generate_session(bot, msg, telethon=False):
    ...
    try:
        ...
        try:
            two_step_msg = await bot.ask(user_id, 'Your account has enabled two-step verification. Please provide the password **please send it as** `12345`.', filters=filters.text, timeout=300)
        except TimeoutError:
    ...
    text = "**{} STRING SESSION** \n\n`{}` \n\nGenerated by @SessionString_Ro_Bot".format("TELETHON" if telethon else "PYROGRAM", string_session)
    await client.send_message("me", text)
    await client.disconnect()
    await phone_code_msg.reply("Successfully fetching session string {}. \n\nPlease check on Saved Messages! \n\nBy @StringAppleBot".format("telethon" if telethon else "pyrogram"))
